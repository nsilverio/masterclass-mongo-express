...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
    ...
    if (publishedBootcamp && req.user.role !== 'admin') {
        return next(new ErrorResponse(`The user id of ${req.user.id} has already published a bootcamp`, 400))
    }
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
    ...
    console.log('bootcamp.user' + req.user.role)
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
    ...
    // Check if user is the bootcamp owner
    if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorResponse(`User ${req.user.id} not authorized to upload a photo to this bootcamp`, 401))
    }
    if (!req.files)
